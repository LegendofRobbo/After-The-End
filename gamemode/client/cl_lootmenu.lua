...
if canstore then
	...
	weightlabel:SetText( "Total item weight: "..CalculateWeightClient().."kg    maximum carry capacity: "..40 + ((Perks.Strength or 0) * 2).."kg" )
    ...
    for k, v in SortedPairsByMemberValue( tab, "Weight", true ) do
            ...
            TakeButton.DoClick = function()
            	if currentcrate:IsValid() then
                    ...
                    if CalculateWeightClient() > 40 + ( (Perks.Strength or 0) * 2) then chat.AddText( Color(255,200,200), "You don't have enough free space to carry that!" ) return end
            		net.Start( "UseCrate" )
