...
ply.Money = ply.Money + math.floor(refund * 0.65)
SystemMessage(ply, "You salvaged one of your props and gained "..math.floor(refund * 0.65).." Gold", Color(205,205,255,255), true)
...
if ptype == 2 and ply:Team() == 1 then SystemMessage(ply, "You must be in a faction to spawn strong props!", Color(255,205,205,255), true) return false end
	...
	local discount = 1 - (ply.StatEngineer * 0.03)
	...
	for k, v in pairs(ents.FindByClass("trader")) do
		if pos:Distance(v:GetPos()) < 400 then SystemMessage(ply, "Unable to spawn prop! Too close to a trader!", Color(255,205,205,255), true) return false end
	end
