...
function LoadLoot()
if not file.IsDir("aftertheend/spawns/loot", "DATA") then
   file.CreateDir("aftertheend/spawns/loot")
end
	if file.Exists( "aftertheend/spawns/loot/" .. string.lower(game.GetMap()) .. ".txt", "DATA" ) then
		LootData = "" --reset it
		LootData = file.Read( "aftertheend/spawns/loot/" .. string.lower(game.GetMap()) .. ".txt", "DATA" )
...
function AddLoot( ply, cmd, args )
	...
	file.Write( "aftertheend/spawns/loot/" .. string.lower(game.GetMap()) .. ".txt", NewData )
	
	SendChat( ply, "Added a loot spawnpoint" )
end
...
if file.Exists(	"aftertheend/spawns/loot/" .. string.lower(game.GetMap()) .. ".txt", "DATA") then
	file.Delete("aftertheend/spawns/loot/" .. string.lower(game.GetMap()) .. ".txt")
end
...
if ((CalculateWeight(ply) + item["Weight"]) > (40 + ((ply.StatStrength or 0) * 2))) then SystemMessage(client, "You don't have enough space for that!", Color(255,205,205,255), true) return end
...
if ent.ContainerRemoveEmpty and table.Count( ent.ContainerItems ) < 1 then ent.ContainerItems = nil timer.Simple(10, function() if ent:IsValid() then ent:Remove() end end) end
