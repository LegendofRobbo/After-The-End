...
if not file.IsDir("aftertheend/spawns/zombies", "DATA") then
   file.CreateDir("aftertheend/spawns/zombies")
end
	if file.Exists( "aftertheend/spawns/zombies/" .. string.lower(game.GetMap()) .. ".txt", "DATA" ) then
		ZombieData = "" --reset it
		ZombieData = file.Read( "aftertheend/spawns/zombies/" .. string.lower(game.GetMap()) .. ".txt", "DATA" )
...
function AddZombie( ply, cmd, args )
	...
	file.Write( "aftertheend/spawns/zombies/" .. string.lower(game.GetMap()) .. ".txt", NewData )
...
if file.Exists(	"aftertheend/spawns/zombies/" .. string.lower(game.GetMap()) .. ".txt", "DATA") then
	file.Delete("aftertheend/spawns/zombies/" .. string.lower(game.GetMap()) .. ".txt")
	ZombieData = ""
end
...
function Payout(ply, xp, cash)
	if( ply:IsPlayer() and ply:IsValid() ) then
		...
		local XPBonus = math.floor( XPGain * ply.StatKnowledge * 0.05 )
		local MoneyBonus = math.floor( MoneyGain * (ply.StatSalvage * 0.05) )
