...
if Config[ "FileSystem" ] == "Legacy" then
	...
	if (file.Exists( "aftertheend/players/" .. string.lower(string.gsub( ply:SteamID(), ":", "_" )) .. "/inventory.txt", "DATA" )) then
		LoadedData = file.Read( "aftertheend/players/" .. string.lower(string.gsub( ply:SteamID(), ":", "_" ) .. "/inventory.txt"), "DATA" )
	end
...
if Config[ "FileSystem" ] == "Legacy" then
	...
	file.Write( "aftertheend/players/" .. string.lower(string.gsub( ply:SteamID(), ":", "_" )) .. "/inventory.txt", data )
elseif Config[ "FileSystem" ] == "PData" then
...
if ((CalculateWeight(ply) + item["Weight"]) > (40 + ((ply.StatStrength or 0) * 2))) then SendChat(ply, "You don't have any inventory space left for this item!") return false end -- dont need to pester them with notifications from a system function, just return false
...
if (cash < (item["Cost"] * (1 - (self.StatBarter * 0.02)))) then SendChat(self, "You cannot afford that!") return false end
if ((CalculateWeight(self) + item["Weight"]) > (40 + ((self.StatStrength or 0) * 2))) then SendChat(self, "You do not have enough space for that!") return false end


SystemGiveItem( self, str )


self.Money = math.floor(self.Money - (item["Cost"] * (1 - (self.StatBarter * 0.02)))) -- reduce buy cost by 2% per barter level
SendInventory(self)
...
local cash = tonumber(client.Money)
	...
	client.Money = math.floor(client.Money + (item["Cost"] * (0.3 + (client.StatBarter * 0.02)))) -- base sell price 30% of the original buy price plus 2% per barter level to max of 50%
	SendInventory(client)
