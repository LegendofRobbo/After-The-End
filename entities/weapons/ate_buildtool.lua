...
if not (CLIENT) then return false end
	...
	surface.DrawText("R: Salvage Prop")
...
self:SetBYaw(0)
self:SetBRoll(0)
self.Owner:ConCommand("play items/ammocrate_open.wav")
...
if caxis == 0 then
		...
		if pitch <= 0 then self:SetBPitch(345) else
		self:SetBPitch(pitch - 15)
		end
elseif caxis == 1 then
		local yaw = self:GetBYaw()
		if yaw <= 0 then self:SetBYaw(345) else
		self:SetBYaw(yaw - 15)
		end
elseif caxis == 2 then
		local roll = self:GetBRoll()
		if roll <= 0 then self:SetBRoll(345) else
		self:SetBRoll(roll - 15)
		end
	end
...
if caxis == 0 then
		local pitch = self:GetBPitch()
		if pitch >= 345 then self:SetBPitch(0) else
		self:SetBPitch(pitch + 15)
	end
elseif caxis == 1 then
		local yaw = self:GetBYaw()
		if yaw >= 345 then self:SetBYaw(0) else
		self:SetBYaw(yaw + 15)
	end
elseif caxis == 2 then
		local roll = self:GetBRoll()
		if roll >= 345 then self:SetBRoll(0) else
		self:SetBRoll(roll + 15)
	end
end
